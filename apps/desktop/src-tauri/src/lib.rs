...
fn init_db(root: &PathBuf) -> Connection {
    ...
    let db = Connection::open(root.join("tender-agent.db")).expect("open sqlite");
    db.execute_batch(
        "
...
fn get_keychain_secret(account: &str) -> Result<String, String> {
    keychain_entry(account)?
        .get_password()
        .map_err(|error| format!("credential not available in OS keychain: {error}"))
}

fn normalize_chat_ids(ids: Vec<String>) -> Vec<String> {
    let mut normalized = Vec::new();
    for value in ids {
        let id = value.trim();
        if !id.is_empty() && !normalized.iter().any(|item| item == id) {
...
fn load_wecom_profile(db: &Connection) -> Result<Option<WeComProfile>, String> {
    db.query_row(
        ...
        |row| {
            ...
            let target_chat_ids = serde_json::from_str::<Vec<String>>(&target_json).unwrap_or_default();
            Ok(WeComProfile {
...
fn redact_value(text: String, bot_id: &str, secret: &str) -> String {
...
fn call_sidecar_once(method: &str, params_value: Value, bot_id: &str, secret: &str) -> Result<Value, String> {
    ...
    let stderr = redact_value(String::from_utf8_lossy(&output.stderr).to_string(), bot_id, secret);
    ...
    if let Some(error) = response.get("error") {
        return Err(redact_value(error.to_string(), bot_id, secret));
    }
    response
        .get("result")
        .cloned()
...
fn fetch_wecom_credentials() -> Result<(String, String), String> {
    Ok((
        get_keychain_secret(WECOM_BOT_ID_ACCOUNT)?,
        get_keychain_secret(WECOM_BOT_SECRET_ACCOUNT)?,
    ))
}
...
fn list_tasks(state: State<AppState>) -> Result<Vec<Task>, String> {
    ...
    let mut statement = db
        .prepare("SELECT t.id,t.name,t.status,t.created_at,COUNT(i.id) query_count FROM tasks t LEFT JOIN task_items i ON i.task_id=t.id GROUP BY t.id ORDER BY t.created_at DESC")
        .map_err(|error| error.to_string())?;
    let rows = statement
        .query_map([], |row| {
            Ok(Task {
...
fn start_task(task_id: String, state: State<AppState>) -> Result<(), String> {
    ...
    log_event(
        ...
        Some(&task_id),
        "INFO",
        "task_execution_requested: sidecar must validate account session before portal access",
    )?;
    Ok(())
}
...
fn get_wecom_bot_status(state: State<AppState>) -> Result<WeComBotStatus, String> {
    ...
    let configured = get_keychain_secret(WECOM_BOT_ID_ACCOUNT).is_ok()
        && get_keychain_secret(WECOM_BOT_SECRET_ACCOUNT).is_ok();
    Ok(match profile {
...
fn configure_wecom_bot(input: ConfigureWeComBotInput, state: State<AppState>) -> Result<WeComBotStatus, String> {
    ...
    let target_chat_ids = normalize_chat_ids(input.target_chat_ids);
    let websocket_url = input.websocket_url.and_then(|value| {
...
fn test_wecom_bot(state: State<AppState>) -> Result<Value, String> {
    ...
    let (bot_id, secret) = fetch_wecom_credentials()?;
...
fn send_wecom_bot_markdown(input: SendWeComBotMarkdownInput, state: State<AppState>) -> Result<Value, String> {
    ...
    let targets = input
        .target_chat_ids
        .map(normalize_chat_ids)
        .unwrap_or(profile.target_chat_ids);
    if targets.is_empty() {
